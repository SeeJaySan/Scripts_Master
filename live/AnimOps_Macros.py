...
class main(object):
    ...
    def __init__(self):
        ...
        self.crvsSelectOnly_bn = mc.button( label='only Select Curves', command=self.selectOnlyCRVs)
        ...
        self.removeInbetween_bn = mc.button( label='- Inbetween', command=self.removeInbetween)
        mc.separator(height = 20)
        mc.text('Graph Editor Key Tools')
        mc.separator(height = 10)
        self.cutKeys_bn = mc.button( label='Cut', command=self.GEcutKeys)
        self.LoopVis_bn = mc.button( label='Visulize Loop', command=self.graphEditorPrepPostLoop)
        mc.separator(height = 20)
        ...
        #display new window
        mc.showWindow()
        mc.window(self.window, e = True, height = self.height, width = self.width, mnb = False, mxb = False)
