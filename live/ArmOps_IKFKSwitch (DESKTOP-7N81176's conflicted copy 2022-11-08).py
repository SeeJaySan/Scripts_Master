...
def ArmOps_IKFKSwitch(side = 'L'):
    ...
    for i in chainList:
        ...
        con = mc.circle(n=i.replace(namereplace, 'CON'), nr=[
                        1, 0, 0], sw=360, r=controlerSize)
        if sdkcheck:
    ...
    IKR_Handle = mc.ikHandle(name='{}_IKR'.format(
    ...
    ik_rotate_control = mc.circle(n='{}_ik_rotate_control'.format(
    ...
    mc.connectAttr(
        ('{}.rotate'.format(fkjoints2)), '{}.color1'.format(mid_blend_color))
    mc.connectAttr(
        ('{}.rotate'.format(ikjoints2)), '{}.color2'.format(mid_blend_color))
    mc.connectAttr(
        ('{}.output'.format(mid_blend_color)), '{}.rotate'.format(driverjoints2))

    # Connecting IKFK blender end
    mc.connectAttr(
        ('{}.rotate'.format(fkjoints3)), '{}.color1'.format(end_blend_color))
    mc.connectAttr(
        ('{}.rotate'.format(ikjoints3)), '{}.color2'.format(end_blend_color))
    mc.connectAttr(
