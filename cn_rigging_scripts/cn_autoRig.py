...
class main(QtWidgets.QDialog):
    ...
    def create_connections(self):
        ...
        self.orientation_cb.currentIndexChanged.connect(self.rig.update_orientation)
        self.up_cb.currentIndexChanged.connect(self.rig.update_up)
...
class TwoBoneIKFK(object):
    ...
    def create_guides(self, newRigtype="none"):
        ...
        if (
            self.prefix == self.prefix_error_check
            and self.rigType == self.rigType_error_check
            and mc.objExists(self.rigPart)
        ):
        ...
        self.arm_guide_loc_group = mc.createNode(
            "transform",
            name="{}_{}_LOC_{}_{}".format(
                self.prefix, self.rigType, self.GUIDE, self.GROUP
            ),
            parent=self.arm_guide_group,
        )
        self.arm_guide_control_group = mc.createNode(
            "transform",
            name="{}_{}_{}_{}_{}".format(
                self.prefix, self.rigType, self.GUIDE, self.CONTROL, self.GROUP
            ),
            parent=self.arm_guide_group,
        )
        ...
        self.root_loc = mc.spaceLocator(
            name="{}_{}_ROOT_{}".format(self.prefix, self.rigType, self.GUIDE)
        )[0]
        self.mid_loc = mc.spaceLocator(
            name="{}_{}_MID_{}".format(self.prefix, self.rigType, self.GUIDE)
        )[0]
        self.end_loc = mc.spaceLocator(
            name="{}_{}_END_{}".format(self.prefix, self.rigType, self.GUIDE)
        )[0]
        ...
        mc.setAttr("{}.t".format(self.root_loc), 0, 0, 0)
        mc.setAttr("{}.t".format(self.mid_loc), 5, 0, 0)
        mc.setAttr("{}.t".format(self.end_loc), 10, 0, 0)

        # create controls to move locators
        self.root_loc_control = mc.circle(
            name="{}_{}_{}_{}_{}".format(
                self.prefix, self.rigType, self.SHOULDER, self.GUIDE, self.CONTROL
            ),
            r=2,
            nr=(1, 0, 0),
        )[0]
        ...
        self.elbow_loc_control = mc.circle(
            name="{}_{}_{}_{}_{}".format(
                self.prefix, self.rigType, self.ELBOW, self.GUIDE, self.CONTROL
            ),
            r=2,
            nr=(1, 0, 0),
        )[0]
        ...
        self.wrist_loc_control = mc.circle(
            name="{}_{}_{}_{}".format(
                self.prefix, self.rigType, self.WRIST, self.GUIDE, self.CONTROL
            ),
            r=2,
            nr=(1, 0, 0),
        )[0]
    ...
    def createHierarchy(self, mirroring=0):
        ...
        if mirroring == 0:
            mc.parent(
                "{}_{}_{}_{}".format(self.prefix, self.rigType, self.GUIDE, self.GROUP),
                self.rigPart,
            )
    ...
    def create_joints(self, mirroring=0):
        ...
        if mirroring == 0:
            ...
            mc.makeIdentity(self.Init_JNT[0], a=True, t=0, r=1, s=0, n=0, pn=True)
            mc.makeIdentity(self.Init_JNT[1], a=True, t=0, r=1, s=0, n=0, pn=True)
            mc.makeIdentity(self.Init_JNT[2], a=True, t=0, r=1, s=0, n=0, pn=True)

            # Orient joints
            mc.joint(
                self.Init_JNT[0],
                edit=True,
                oj=self.jointOrientation,
                sao=self.orientationUp,
                ch=True,
                zso=True,
            )
            ...
            om.MGlobal_displayWarning("Check Joint Orientations before proceeding")
    ...
    def mirroring_joints(self):
        ...
        mirror_joint = mc.joint(
            n="C_{}_mirror_dummie_{}".format(self.rigType, self.JOINT)
        )
    ...
    def get_bnd_joint_data(self, mirroring=0):
        ...
        if mirroring == 1:
            ...
            self.Bnd1_pos = mc.xform(self.mirrored_joints[0], q=True, t=True, ws=True)
            self.Bnd2_pos = mc.xform(self.mirrored_joints[1], q=True, t=True, ws=True)
            self.Bnd3_pos = mc.xform(self.mirrored_joints[2], q=True, t=True, ws=True)

            self.Bnd1_mat = mc.xform(self.mirrored_joints[0], q=True, m=True, ws=True)
            self.Bnd2_mat = mc.xform(self.mirrored_joints[1], q=True, m=True, ws=True)
            self.Bnd3_mat = mc.xform(self.mirrored_joints[2], q=True, m=True, ws=True)
    ...
    def ikfkProcessdure(self, mirroring=0):
        ...
        if mirroring == 0:
            ...
            self.bn_joint_1 = mc.duplicate(
                self.Init_JNT[0],
                n="{}".format(self.Init_JNT[0]).replace("Skin", "BN"),
                po=1,
            )
            mc.setAttr(self.bn_joint_1[0] + ".radius", self.joint_radius)

            self.bn_joint_2 = mc.duplicate(
                self.Init_JNT[1],
                n="{}".format(self.Init_JNT[1]).replace("Skin", "BN"),
                po=1,
            )
            mc.setAttr(self.bn_joint_2[0] + ".radius", self.joint_radius)

            self.bn_joint_3 = mc.duplicate(
                self.Init_JNT[2],
                n="{}".format(self.Init_JNT[2]).replace("Skin", "BN"),
                po=1,
            )
        ...
        if mirroring == 1:
            self.joint_radius = mc.getAttr((self.mirrored_joints[0] + ".radius"))
            self.joint_radius = int(self.joint_radius) / 2

            self.bn_joint_1 = mc.duplicate(
                self.mirrored_joints[0],
                n="{}".format(self.mirrored_joints[0]).replace("Skin", "BN"),
                po=1,
            )
            mc.setAttr(self.bn_joint_1[0] + ".radius", self.joint_radius)

            self.bn_joint_2 = mc.duplicate(
                self.mirrored_joints[1],
                n="{}".format(self.mirrored_joints[1]).replace("Skin", "BN"),
                po=1,
            )
            mc.setAttr(self.bn_joint_2[0] + ".radius", self.joint_radius)

            self.bn_joint_3 = mc.duplicate(
                self.mirrored_joints[2],
                n="{}".format(self.mirrored_joints[2]).replace("Skin", "BN"),
                po=1,
            )
            mc.setAttr(self.bn_joint_3[0] + ".radius", self.joint_radius)
            mc.select(cl=1)
        ...
        # Create FK joints
        self.fk_joint_1 = mc.duplicate(
            self.bn_joint_1[0],
            n="{}".format(self.bn_joint_1[0]).replace("BN", "FK"),
            po=1,
        )
        mc.setAttr(self.fk_joint_1[0] + ".radius", self.joint_radius * 1.5)

        self.fk_joint_2 = mc.duplicate(
            self.bn_joint_2[0],
            n="{}".format(self.bn_joint_2[0]).replace("BN", "FK"),
            po=1,
        )
        mc.setAttr(self.fk_joint_2[0] + ".radius", self.joint_radius * 1.5)

        self.fk_joint_3 = mc.duplicate(
            self.bn_joint_3[0],
            n="{}".format(self.bn_joint_3[0]).replace("BN", "FK"),
            po=1,
        )
        ...
        # Create IK joints
        self.ik_joint_1 = mc.duplicate(
            self.bn_joint_1[0],
            n="{}".format(self.bn_joint_1[0]).replace("BN", "IK"),
            po=1,
        )
        mc.setAttr(self.ik_joint_1[0] + ".radius", self.joint_radius * 0.5)

        self.ik_joint_2 = mc.duplicate(
            self.bn_joint_2[0],
            n="{}".format(self.bn_joint_2[0]).replace("BN", "IK"),
            po=1,
        )
        mc.setAttr(self.ik_joint_2[0] + ".radius", self.joint_radius * 0.5)

        self.ik_joint_3 = mc.duplicate(
            self.bn_joint_3[0],
            n="{}".format(self.bn_joint_3[0]).replace("BN", "IK"),
            po=1,
        )
        ...
        self.root_blend_color = mc.createNode(
            "blendColors",
            name="{}_{}_{}_BLENDECOLOR".format(self.prefix, self.rigType, self.ROOT),
        )
        self.mid_blend_color = mc.createNode(
            "blendColors",
            name="{}_{}_{}_BLENDECOLOR".format(self.prefix, self.rigType, self.MID),
        )
        self.end_blend_color = mc.createNode(
            "blendColors",
            name="{}_{}_{}_BLENDECOLOR".format(self.prefix, self.rigType, self.END),
        )
    ...
    def create_fk_controls(self):
        ...
        self.fk_controls_group = mc.createNode(
            "transform",
            name="{}_{}_FKControls_{}".format(self.prefix, self.rigType, self.GROUP),
        )
        ...
        self.root_controls_offset = mc.createNode(
            "transform",
            name="{}_{}_{}_{}".format(
                self.prefix, self.rigType, self.ROOT, self.OFFSET
            ),
            parent=self.fk_controls_group,
        )
        self.root_controls_sdk = mc.createNode(
            "transform",
            name="{}_{}_{}_{}".format(self.prefix, self.rigType, self.ROOT, self.SDK),
            parent=self.root_controls_offset,
        )

        self.root_control = mc.circle(
            name="{}_{}_{}_{}".format(
                self.prefix, self.rigType, self.ROOT, self.CONTROL
            ),
            nr=self.fk_control_shape_orientation,
        )[0]
        ...
        self.mid_controls_offset = mc.createNode(
            "transform",
            name="{}_{}_{}_{}".format(self.prefix, self.rigType, self.MID, self.OFFSET),
            parent=self.root_control,
        )
        self.mid_controls_sdk = mc.createNode(
            "transform",
            name="{}_{}_{}_{}".format(self.prefix, self.rigType, self.MID, self.SDK),
            parent=self.mid_controls_offset,
        )

        self.mid_control = mc.circle(
            name="{}_{}_{}_{}".format(
                self.prefix, self.rigType, self.MID, self.CONTROL
            ),
            nr=self.fk_control_shape_orientation,
        )[0]
        ...
        self.end_controls_offset = mc.createNode(
            "transform",
            name="{}_{}_{}_{}".format(self.prefix, self.rigType, self.END, self.OFFSET),
            parent=self.mid_control,
        )
        self.end_controls_offset = mc.createNode(
            "transform",
            name="{}_{}_{}_{}".format(self.prefix, self.rigType, self.END, self.SDK),
            parent=self.end_controls_offset,
        )
        self.end_control = mc.circle(
            name="{}_{}_{}_{}".format(
                self.prefix, self.rigType, self.END, self.CONTROL
            ),
            nr=self.fk_control_shape_orientation,
        )[0]
    ...
    def create_ik_controls(self):
        ...
        self.IK_wrist_GRP = mc.createNode(
            "transform",
            name="{}_{}_IKControls_{}".format(self.prefix, self.rigType, self.GROUP),
        )
        self.IK_controls_offset = mc.createNode(
            "transform",
            name="{}_{}_{}_{}".format(self.prefix, self.WRIST, self.OFFSET, self.GROUP),
            parent=self.IK_wrist_GRP,
        )
        ...
        self.IKR_Handle = mc.ikHandle(
            name="{}_{}_IKR".format(self.prefix, self.rigType),
            sol="ikRPsolver",
            sj=self.ik_joint_1[0],
            ee=self.ik_joint_3[0],
        )[0]

        # Create Control
        self.IKR_wrist_control = mc.curve(
            n="{}_{}_{}".format(self.prefix, self.rigType, self.CONTROL),
            d=1,
            p=zbw_con.shapes["cube"],
        )
    ...
    def create_pole_target(self, distance=1.0):
        ...
        def create_pv(pos):
            ...
            pv_group = mc.createNode(
                "transform",
                name="{}_{}_PV_{}".format(self.prefix, self.rigType, self.GROUP),
            )

            self.pv_control = mc.curve(
                n="{}_{}_PV_{}".format(self.prefix, self.rigType, self.CONTROL),
                d=1,
                p=zbw_con.shapes["cube"],
            )
            ...
            mc.poleVectorConstraint(self.pv_control, self.IKR_Handle)
    ...
    def create_twist_joints(self):
        ...
        root_twist_xform = mc.xform(self.Init_JNT[0], q=True, ws=True, t=True)
        mid_twist_xform = mc.xform(self.Init_JNT[1], q=True, ws=True, t=True)
        end_twist_xform = mc.xform(self.Init_JNT[2], q=True, ws=True, t=True)
        ...
        self.twist_joint_rtm_ik_1 = mc.joint(
            n="{}_{}_{}_Twist_Skin{}_{}".format(
                self.prefix, self.rigType, self.RTM, self.ROOT, self.JOINT
            ),
            rad=self.joint_radius * 2,
        )
        self.twist_joint_rtm_ik_2 = mc.joint(
            n="{}_{}_{}_Twist_Skin{}_{}".format(
                self.prefix, self.rigType, self.RTM, self.MID, self.JOINT
            ),
            rad=self.joint_radius * 2,
        )
        mc.select(cl=True)
        self.twist_joint_mte_ik_1 = mc.joint(
            n="{}_{}_{}_Twist_Skin{}_{}".format(
                self.prefix, self.rigType, self.MTE, self.MID, self.JOINT
            ),
            rad=self.joint_radius * 2,
        )
        self.twist_joint_mte_ik_2 = mc.joint(
            n="{}_{}_{}_Twist_Skin{}_{}".format(
                self.prefix, self.rigType, self.MTE, self.END, self.JOINT
            ),
            rad=self.joint_radius * 2,
        )
        ...
        for i in range(self.twistJointCount):
            ...
            # Cycle through the positions
            joint_location = (
                mid_to_end_vec - ((mid_to_end_vec / tJC) * mte_multiple)
            ) + mid_twist_vec
    ...
    def create_asset_and_attributes(self):
        ...
        mc.container(
            self.arm_attributes_asset,
            e=True,
            ish=True,
            f=True,
            an=self.ikfk_attributes_Grp,
        )
    ...
    def make_connections(self):
        ...
        mc.container(self.arm_attributes_asset, edit=True, addNode=self.root_control)
        mc.container(self.arm_attributes_asset, edit=True, addNode=self.mid_control)
        mc.container(self.arm_attributes_asset, edit=True, addNode=self.end_control)
        mc.container(
            self.arm_attributes_asset, edit=True, addNode=self.IKR_wrist_control
        )
        mc.container(self.arm_attributes_asset, edit=True, addNode=self.pv_control)
        ...
        mc.container(
            self.arm_attributes_asset,
            e=True,
            ba=(self.ikfk_attributes_Grp + ".IKFK_Switch", "IKFK_Switch"),
        )

        # Connect attribute to the reverse
        mc.connectAttr(
            self.ikfk_attributes_Grp + ".IKFK_Switch", IKFK_reverse + ".input.inputX"
        )

        # Connect the reverse to the blend color nodes
        mc.connectAttr(
            IKFK_reverse + ".input.inputX", self.root_blend_color + ".blender"
        )
        mc.connectAttr(
            IKFK_reverse + ".input.inputX", self.mid_blend_color + ".blender"
        )
        mc.connectAttr(
            IKFK_reverse + ".input.inputX", self.end_blend_color + ".blender"
        )

        # Create visibility depending on ik or fk
        self.arm_control_vis_condition = mc.shadingNode(
            "condition",
            n="{}_{}_control_vis_condition".format(self.prefix, self.rigType),
            au=True,
        )
        ...
        mc.connectAttr(
            self.arm_attributes_asset + ".IKFK_Switch",
            self.arm_control_vis_condition + ".firstTerm",
        )

        mc.connectAttr(
            self.arm_control_vis_condition + ".outColor.outColorR",
        ...
        mc.connectAttr(
            self.arm_control_vis_condition + ".outColor.outColorG",
    ...
    def IKFK_snap_expression(self):  # NOT WORKING
        ...
        scale_distance_node = mc.createNode(
            "distanceBetween",
            n="{}_{}_distanceBetween".format(self.prefix, self.rigType),
        )
        mc.addAttr(
            self.IKR_wrist_control, at="float", ln="Stretch", k=True, min=0, max=1
        )
        mc.addAttr(self.IKR_wrist_control, at="float", ln="ShortArm", k=True)
        mc.expression(
            n="{}_{}_IkfkSnap".format(self.prefix, self.rigType),
            ae=True,
            s="""
                      $distance = {0}.distance ;
                      $initialDis = {1};
