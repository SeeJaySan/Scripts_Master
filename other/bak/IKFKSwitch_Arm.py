...
for i in chainList:
    ...
    con = mc.circle(n = i.replace(namereplace, 'CON'), nr = [1, 0, 0], sw = 360, r = controlerSize)
    if sdkcheck:
...
IKR_Handle = mc.ikHandle(name='{}_IKR'.format(name), sol='ikRPsolver', sj=ikjoints1, ee=ikjoints3)[0]
...
# Parenting
mc.xform(IKR_wrist_control, m=BN3_mat, ws=True)
...
mc.orientConstraint(IKR_wrist_control, ikjoints3)


# Create Blend Colors

root_blend_color = mc.createNode('blendColors', name='{}_Shoulder_BLENDECOLOR'.format(name))
mid_blend_color = mc.createNode('blendColors', name='{}_Elbow_BLENDECOLOR'.format(name))
end_blend_color = mc.createNode('blendColors', name='{}_Wrist_BLENDECOLOR'.format(name))
...
mc.connectAttr(('{}.rotate'.format(fkjoints1)),'{}.color1'.format(root_blend_color))
mc.connectAttr(('{}.rotate'.format(ikjoints1)),'{}.color2'.format(root_blend_color))
mc.connectAttr(('{}.output'.format(root_blend_color)), '{}.rotate'.format(driverjoints1))
...
mc.connectAttr(('{}.output'.format(mid_blend_color)), '{}.rotate'.format(driverjoints2))
...
mc.connectAttr(('{}.output'.format(end_blend_color)), '{}.rotate'.format(driverjoints3))
...
ikfk_attributes_Grp = mc.createNode('transform', name='{}_ATRIBUTES_GRP'.format(name))
...
mc.container(arm_attributes_asset, e=True, ba=(ikfk_attributes_Grp + '.IKFK_Switch', 'IKFK_Switch'))
